...
private static <S extends GameState<S, A>, A extends GameAction<S,A>> void startGUIMode(final String gType, final GameTable<S, A> game, String playerModes[])
{
		...
		for(int i = 0; i < game.getState().getPlayerCount(); i++)
		{
			...
			final GamePlayer smart = new SmartPlayer("Player - " + i,4);
			random.join(i);
			smart.join(i);
			final int j = i;
			try {
				SwingUtilities.invokeAndWait(new Runnable(){
					public void run()
					{
						//new GameWindow<S,A>(j,random,smart,vistaJuego,controlador,game);
						new GameWindow<S,A>(j,random,smart,(GameView<S,A>)createGameView(gType,j),controlador,game);
					}
				});
